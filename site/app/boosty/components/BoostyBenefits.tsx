...
function PlanHeading({ plan }: { plan: BoostyPlan }) {
  return (
    <div className="boosty-plan-heading" style={{ "--plan-color": plan.color } as React.CSSProperties}>
      <span>{plan.name}</span>
  ...
  return (
    <>
      <section className="boosty-hero">
        <a
          className="boosty-external-button"
          href={boostyUrl}
          target="_blank"
          rel="noreferrer"
        >
        ...
        <div className="boosty-hero-copy">
          <span className="boosty-kicker">Поддержка сообщества</span>
          <h1>Больше возможностей вместе с Linken&apos;s Sphere</h1>
          <p>
            Выберите подходящий уровень поддержки, получите особую роль в Discord
            и дополнительные возможности в событиях сообщества.
          </p>
        </div>
      </section>

      <section className="boosty-benefits-section" aria-labelledby="boosty-benefits-title">
        ...
        <div className="boosty-benefits-matrix" role="table" aria-label="Уровни и преимущества Boosty">
              ...
              {boostyPlans.map((plan) => (
                <span role="cell" key={plan.id}>
                  {plan.benefitIds.includes(benefit.id) && (
                    <BenefitCheck label={`${plan.name}: преимущество доступно`} />
                  )}
                </span>
              ))}
            </div>
