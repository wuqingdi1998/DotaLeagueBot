...
export function useFearlessDraft(initialSnapshot: FearlessDraftSnapshot) {
  ...
  const reload = useCallback(async () => {
    const response = await fetch("/api/fearless-draft", { cache: "no-store" });
  ...
  useEffect(() => {
    const events = new EventSource("/api/fearless-draft/events");
