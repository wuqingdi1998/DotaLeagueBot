...
def test_archived_league_interfaces_do_not_register_debug_fill_commands() -> None:
    archived_interfaces = (
        PROJECT_ROOT / "bot" / "legacy" / "league.py",
        PROJECT_ROOT / "bot" / "legacy" / "seasonal_league.py",
    )

    for interface_path in archived_interfaces:
        source = interface_path.read_text(encoding="utf-8")
        assert 'command(name="debug_fill"' not in source, interface_path
